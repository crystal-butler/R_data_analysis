...
# logical combination of upright and Word responses 
upWord=lab3clean[lab3clean$"Response Type"==WORD & lab3clean$Rotation==NORMAL, ]

# to list all Words, use Words() in R console
# To plot a histrogram just use the hist() function

# Plot individual Upright Word histograms
# to list all Words, use Words() in R console
...
dev.new()
hist(upWordRTCON, breaks=seq(0,max(upWordRTCON)+50,50), col="slateblue4", ylim=c(0,100), xlim=c(0,4000),main="", xlab="Reaction Time in Milliseconds, Upright Trials", ylab="Response Frequency")
...
# To plot a histrogram just use the hist() function. add=T adds to previous plot.
dev.new()
hist(upWordRTINCON, breaks=seq(0,max(upWordRTINCON)+50,50), col="khaki3", ylim=c(0,100), xlim=c(0,4000),main="", xlab="Reaction Time in Milliseconds, Upright Trials", ylab="Response Frequency")


# Plot all Upright Word trials on a single histogram
# To plot a histrogram just use the hist() function
dev.new()
hist(upWordRTCON, breaks=seq(0,max(upWordRTCON)+50,50), col="ivory1", ylim=c(0,100), xlim=c(0,4000),main="", xlab="Reaction Time in Milliseconds, Upright Trials", ylab="Response Frequency")
...
dev.new()
conBars = barplot(subjMed, col=c("ivory1", "grey63"), main="", xlab="Condition, Word Naming Upright Trials", ylab="RT in Milliseconds", ylim=c(0, max(subjMed)+max(sErrs2)+100))
...
# Plot all Upright Word trials on a single histogram for Accuracy
...
# logical combination of upright and Word responses 
upWord=lab3data[lab3data$"Response Type"==WORD & lab3data$Rotation==NORMAL, ]
...
main="", xlab="Condition, Word Naming Upright Trials", ylab="Mean Percent Accurate Responses", ylim=c(0, 100))
