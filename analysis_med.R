...
dev.new()
hist(upColorRTCON, breaks=seq(0,max(upColorRTCON)+50,50), col="slateblue4", ylim=c(0,100), xlim=c(0,4000),main="", xlab="Reaction Time in Milliseconds, Upright Trials", ylab="Response Frequency")
...
# To plot a histrogram just use the hist() function. add=T adds to previous plot.
dev.new()
hist(upColorRTINCON, breaks=seq(0,max(upColorRTINCON)+50,50), col="khaki3", ylim=c(0,100), xlim=c(0,4000),main="", xlab="Reaction Time in Milliseconds, Upright Trials", ylab="Response Frequency")


# Plot all Upright Color trials on a single histogram
# To plot a histrogram just use the hist() function
dev.new()
hist(upColorRTCON, breaks=seq(0,max(upColorRTCON)+50,50), col="ivory1", ylim=c(0,100), xlim=c(0,4000),main="", xlab="Reaction Time in Milliseconds, Upright Trials", ylab="Response Frequency")

# To plot a histrogram just use the hist() function. add=T adds to previous plot.
hist(upColorRTCTL, breaks=seq(0, max(upColorRTCTL)+50,50), col="grey63", ylim=c(0,100), xlim=c(0,4000), add=T)

# To plot a histrogram just use the hist() function. add=T adds to previous plot.
hist(upColorRTINCON, breaks=seq(0,max(upColorRTINCON)+50,50), col="grey31", ylim=c(0,100), xlim=c(0,4000), add=T)
...
sDevs2= apply(upColorRTmed, 2, sd)
print (sDevs2)
...
dev.new()
conBars = barplot(subjMed, col=c("ivory1", "grey63", "grey31"), main="", xlab="Condition, Color Naming Upright Trials", ylab="RT in Milliseconds", ylim=c(0, max(subjMed)+max(sErrs2)+100))
segments(conBars, subjMed-sErrs2, conBars, subjMed+sErrs2)

# Reformat data for ANOVA
found = which(upColorRTmed !=-999, arr.ind=T)
rtANOVA = data.frame(cbind(found, upColorRTmed[found]))
...
t.test(upColorRTmed$Control, upColorRTmed$Congruent, paired=T, mu=0, alternative="two.sided", var.equal=T)
t.test(upColorRTmed$Control, upColorRTmed$Incongruent, paired=T, mu=0, alternative="two.sided", var.equal=T)
t.test(upColorRTmed$Congruent, upColorRTmed$Incongruent, paired=T, mu=0, alternative="two.sided", var.equal=T)
...
# Plot all Upright Color trials on a single histogram for Accuracy
...
# get the trials where the stimulus was upright
upResp=lab3data[lab3data$Rotation==NORMAL, ]

# logical combination of upright and color responses 
upColor=lab3data[lab3data$"Response Type"==COLOR & lab3data$Rotation==NORMAL, ]

# Calculate mean and median of reaction time by condition
accbyCondmean=tapply(upColor$Hit, list(upColor$"Subject", upColor$"Trial Type"), mean)
accbyCondmed=tapply(upColor$Hit, list(upColor$"Subject", upColor$"Trial Type"), median)
...
main="", xlab="Condition, Color Naming Upright Trials", ylab="Mean Percent Accurate Responses", ylim=c(0, 100))
segments(conBars, errLow, conBars, errHigh)

accT=t.test(subjMeanAcc, mu=25)
